...
We show how we perform differential expression analysis on all transcripts using empirical p-value (quantile empirical p-values).
To generate p-values under the null, we create a ``residual permuted`` trait 100 times, perform differential expression analysis, and use the resulting p-values as our null p-values. However, users also can implement Storey empirical p-value using test statistcs, as these are referred to in the manuscript.
